...
const STATUS_STYLE: Record<string, string> = {
  ...
  DEVELOPING: "bg-breaking text-white",
  RUMOR: "border border-breaking text-breaking",
...
function KeyPointsBox({ article }: { article: Article }) {
  ...
  return (
    <aside className="my-6 not-prose border border-hair bg-mist/40 p-5">
    ...
    <aside className="my-6 not-prose border-l-4 border-hair pl-4">
      ...
      <ul className="space-y-1">
        {article.sightings.map((s, i) => (
          <li key={i} className="font-body text-[1.02rem] leading-snug text-navy">
      ...
      {s.castAdded?.length ? (
        <div className="mt-3 border-t border-hair pt-3">
          ...
          <ul className="space-y-0.5">
            {s.castAdded.map((c, i) => (
              <li key={i} className="font-body text-[1.02rem] text-navy">
...
function SeriesContextCard({ article }: { article: Article }) {
  ...
  return (
    <aside className="my-8 not-prose border border-hair bg-mist/30 p-5">
...
function CreditsBlock({ article }: { article: Article }) {
  ...
  if (!rows.length) return null;
  return (
    <section className="mt-10 not-prose border-t-2 border-navy pt-4">
...
function VerdictBoxWatch({ article }: { article: Article }) {
  ...
  const winRows = w
    ...
    : [];
  return (
    <aside className="my-6 not-prose border-2 border-navy p-5">
      <SectionLabel>Where to Watch — The Short Answer</SectionLabel>
      {v?.answer ? <p className="font-body text-xl leading-snug text-navy sm:text-2xl">{v.answer}</p> : null}
      {(v?.where || v?.when) ? (
      ...
      {winRows.length ? (
        <dl className="mt-4 divide-y divide-hair border-t border-hair pt-2">
          {winRows.map(([k, val]) => (
            <div key={k} className="flex gap-3 py-1.5">
              <dt className="w-28 flex-none font-sans text-xs font-bold uppercase tracking-[0.04em] text-slate">{k}</dt>
              <dd className="font-body text-[1.02rem] text-navy">{val}</dd>
            </div>
          ))}
        </dl>
      ) : null}
...
function Criterion({ article }: { article: Article }) {
  ...
  return (
    <aside className="my-5 not-prose border-l-4 border-breaking pl-4">
      <SectionLabel>How We Ranked Them</SectionLabel>
      <p className="font-body text-[1.05rem] leading-snug text-navy">{article.criterion}</p>
    </aside>
  );
}
      ...
      <dl className="space-y-1.5">
        {rows.map(([k, v]) => (
          <div key={k} className="flex flex-col gap-0.5 sm:flex-row sm:gap-3">
            ...
            <dd className="font-body text-[1.05rem] text-navy">{v}</dd>
          </div>
        ))}
      </dl>
    </aside>
...
const BUCKET_STYLE: Record<string, string> = {
  FRONTRUNNER: "border-breaking",
  "IN THE HUNT": "border-navy",
    ...
    <div className="my-6 not-prose grid gap-4 sm:grid-cols-2">
      {article.verdictBuckets.map((b, i) => (
        <div key={i} className={`border-l-4 pl-4 ${BUCKET_STYLE[(b.bucket || "").toUpperCase()] || "border-hair"}`}>
          <div className="font-sans text-[11px] font-bold uppercase tracking-[0.1em] text-breaking">{b.bucket}</div>
          <div className="font-body text-[1.05rem] font-semibold leading-snug text-navy">
...
function PrecursorTimeline({ article }: { article: Article }) {
  ...
  return (
    <section className="mt-10 not-prose">
        ...
        <>
          ...
          <ol className="space-y-1.5">
            {article.precursorTimeline.map((p, i) => (
              <li key={i} className="flex gap-3 font-body text-[1.05rem] leading-snug text-navy">
      ...
      {article.bottomLine ? (
        <p className="mt-5 border-t-2 border-navy pt-4 font-body text-xl leading-snug text-navy">
          <span className="font-sans text-xs font-bold uppercase tracking-[0.1em] text-breaking">The Bottom Line — </span>
          {article.bottomLine}
        </p>
      ) : null}
    </section>
      ...
      <ul className="space-y-1.5">
        {article.looseThreads.map((t, i) => (
          <li key={i} className="flex gap-2 font-body text-[1.02rem] leading-snug text-navy">
            <span className="flex-none text-breaking">▸</span>
            <span>{t}</span>
          </li>
        ))}
      </ul>
        ...
        <div className="grid grid-cols-2 gap-px overflow-hidden border border-hair bg-hair sm:grid-cols-4">
          {article.careerStats.map((s, i) => (
            <div key={i} className="bg-white px-4 py-3 text-center">
              <div className="font-display text-2xl font-bold leading-tight text-navy">{s.value}</div>
      ...
      <dl className="space-y-1.5">
        {article.footnotes.map((f, i) => (
          <div key={i} className="font-body text-[1.0rem] leading-snug">
            <dt className="inline font-semibold text-navy">{f.term}: </dt>
