...
- HOOK-FIRST LEDE (owner 2026-07-18 — the flat-lede audit): NEVER open the article with the attribution formula ("X discusses Y, according to Z" / "According to Z, …"). Sentence 1 is the HOOK — the boldest verified fact, the spikiest verbatim quote fragment, or the stakes ("The crew burst into applause…", "\"I nearly walked off,\" X said…"). The outlet attribution moves to sentence 2 or 3 ("…she told The Hollywood Reporter"). For quote-news, a short VERBATIM quote fragment may lead the headline itself ("Zendaya Slams 'Lazy' Odyssey Comparisons") — quote fragments in headlines must be word-for-word from the source.
- TENSION-FIRST FRAMING (owner 2026-07-17, scoped 2026-07-19 — a LITTLE spice, never manufactured, and only about the WORK): this lane covers industry news (a film greenlit, a series ordered, casting, a director attached, deals, trailers) plus work-related conflict (a feud over a project, a firing, a recast, casting backlash). It does NOT cover celebrity opinion/statement pieces on politics or social issues — a separate automation owns those, so never steer a story that way. Within that scope: when the sourced facts contain a GENUINE point of tension — a bold or risky bet, a pointed quote, a split reaction, a rivalry, a surprise reversal, real stakes ("$250M gamble", "replaces X after the on-set injury") — LEAD with that tension in the headline, dek and opening line instead of the flat announcement, and pick the spikiest VERBATIM quote the source offers. The spice must come 100% from the verified facts: NEVER invent or exaggerate a conflict, NEVER editorialize beyond what the source supports, NEVER frame something benign as a scandal, no rage-bait, no question-mark bait. A flat story with no real tension stays a clean, flat story — forced drama reads fake and kills trust.
...
Return JSON with EXACTLY these fields:
{
 ...
 "metaTitle": "the SEO <title> — SEPARATE from the on-page headline (that stays full + expressive). LENGTH 45–55 CHARACTERS (minimum 45, maximum 55 — use full names / the project title so it is NOT too short, but NEVER exceed 55). Do NOT append the site/brand name (the site adds branding automatically). FRONT-LOAD the searchable ENTITY (the film/show/studio name OR the newsmaker), THEN the concrete NEWS VERB (cast, joins, sets release, drops trailer, wins, dies). Phrase it the way a person would GOOGLE it. Accuracy first — NO clickbait, no question mark, no '(EXCLUSIVE)'. It must read as a COMPLETE phrase: NEVER end on a dangling connector ('…in Netflix's The', '…with'), and if you name a person use BOTH names ('Margot Robbie', never a trailing '…with Margot'). PRESERVE the exact source spelling of every title/name — never 'correct' an unusual proper noun. Example — headline \"Zendaya Joins Christopher Nolan's Epic 'The Odyssey' (2026)\" → metaTitle \"Zendaya Cast in Christopher Nolan's 'The Odyssey'\" (49 chars).",
 ...
 "metaDescription": "140-160 chars, keyword early, MUST end on a complete sentence — never trail off mid-thought",
