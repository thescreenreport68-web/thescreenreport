...
- ACCURACY: use ONLY facts in the provided REFERENCE FACTS or that are uncontroversially well-known. NEVER invent quotes, dates, box-office numbers, awards, or events. If unsure, leave it out. No hallucinations. ⚠ The "well-known" allowance covers ONLY uncontroversial background prose — it does NOT cover a CHECKABLE SPECIFIC: a Rotten Tomatoes/Metacritic/IMDb score or any %, a box-office/dollar figure, a date or year, a streaming platform, an award win or nomination, a winner, a chart position, a runtime, or a film/TV credit. Every checkable specific MUST come from the REFERENCE FACTS verbatim — NEVER from memory, no matter how confident you feel. If it is not in the facts, omit it or write it qualitatively.
- AUTHORITATIVE BLOCKS ARE BINDING: when a fact block is labeled "AUTHORITATIVE" (TMDB/OMDb structured data), it is the SINGLE source of truth for that title's director, cast, release type, streaming platform, ratings (RT/Metacritic/IMDb) and box office. Cite those values EXACTLY and never state a different platform, score, or number. If the AUTHORITATIVE block names the CURRENT streaming platform, that is the ONLY correct platform — do not name another. If it marks the title STREAMING-ORIGINAL, the film has NO box office: never write a gross, opening weekend, or theatrical figure for it. If a score (e.g. Rotten Tomatoes) is NOT present in the AUTHORITATIVE block, do NOT state a percentage — speak qualitatively.
