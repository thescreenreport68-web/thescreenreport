...
// ── SPECULATION DESK behavior: a CONTRADICTED (wrong) claim is CUT; a merely UNSUPPORTED speculation is KEPT ──
const bodyFiller = Array.from({ length: 10 }, (_, i) => `Fans online shared reaction number ${i + 1}, dissecting the timing and the body language for any hint of what might really be going on.`).join(" ");
{
  // contradicted:true = the bundle says otherwise (a WRONG fact) → must be cut, even on a speculation desk.
  const dirty = () => ({ ...cleanArticle(), body: "According to People, the pair were spotted together at a Los Angeles restaurant this weekend. They secretly eloped in Vegas last night, a claim the bundle directly contradicts. " + bodyFiller });
  const verifyImpl = async () => ({ ok: false, unsupported: [{ claim: "They secretly eloped in Vegas last night, a claim the bundle directly contradicts", why: "the bundle says they are not married", contradicted: true }], severity: "minor", brokenRatio: 0.1, degraded: false });
  ...
  check("a CONTRADICTED (wrong) claim is CUT from the published body", r.status === "PUBLISH" && !/secretly eloped/i.test(r.article.body), JSON.stringify(r.status) + " " + (r.article?.body || "").slice(0, 60));
}
{
  // contradicted:false = unsupported SPECULATION → this is a gossip desk, so it is KEPT (the writer hedges it), NOT cut.
  const spec = () => ({ ...cleanArticle(), body: "According to People, the pair were spotted together this weekend. Reportedly, an engagement could be on the horizon, fans speculate. " + bodyFiller });
  const verifyImpl = async () => ({ ok: false, unsupported: [{ claim: "an engagement could be on the horizon", why: "not stated in the bundle", contradicted: false }], severity: "minor", brokenRatio: 0.1, degraded: false });
  const r = await runGossip(TOPIC, { writeImpl: async () => spec(), verifyImpl, judgeImpl: async () => ({ score: 80, subscores: { safety: 9 }, issues: [] }), verify: true, judge: true, corroborate: false });
  check("an UNSUPPORTED speculation is KEPT (hedged, not cut) and still publishes", r.status === "PUBLISH" && /engagement could be on the horizon/i.test(r.article.body), JSON.stringify(r.status));
}
