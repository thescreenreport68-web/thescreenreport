...
// ── WS1: CAPTION SAFETY — a source photo is captioned with WHO VISION SAW, never the (possibly wrong) topic entity ──
{
  const ogImpl = async (url) => url.includes("pagesix") ? "https://cdn.pagesix.com/two-shot.jpg" : null;
  const bundle = { sources: [{ outlet: "Page Six", url: "https://pagesix.com/x", tier: 6 }] };
  // topic says primaryEntity "Taylor Swift" but the story (and photo) is about Abigail Anderson
  const h = await pickHero({ topic: { primaryEntity: "Taylor Swift", coSubjects: ["Abigail Anderson"], title: "x", gossipType: "general" }, article: { title: "x" }, bundle },
    { getPersonImagesImpl, getTitleImagesImpl: async () => null, visionImpl: async () => ({ ranked: [{ index: 0, usable: true, impact: 8, fit: 8, who: "Abigail Anderson" }] }), ogImpl });
  check("source photo caption names WHO VISION SAW (Abigail), not the topic entity (Taylor)", /Pictured: Abigail Anderson/.test(h.caption) && !/Taylor/.test(h.caption), h.caption);
}
{
  // vision ERRORS on the source photo → keep the outlet photo but caption stays NEUTRAL (no confident wrong name)
  const ogImpl = async (url) => url.includes("pagesix") ? "https://cdn.pagesix.com/x.jpg" : null;
  const bundle = { sources: [{ outlet: "Page Six", url: "https://pagesix.com/x", tier: 6 }] };
  const h = await pickHero({ topic: { primaryEntity: "Taylor Swift", title: "x", gossipType: "general" }, article: { title: "x" }, bundle },
    { getPersonImagesImpl, getTitleImagesImpl: async () => null, visionImpl: async () => { throw new Error("vision down"); }, ogImpl });
  check("vision error on source photo → NEUTRAL caption (no 'Pictured: X')", h.source === "source" && !/Pictured:/.test(h.caption || ""), JSON.stringify(h.caption));
}
{
  // a WRONG-PERSON / generic source photo (vision usable=false) → rejected → falls to TMDB (never published as-is)
  const ogImpl = async () => "https://cdn.pagesix.com/generic-model.jpg";
  const bundle = { sources: [{ outlet: "Page Six", url: "https://pagesix.com/x", tier: 6 }] };
  const h = await pickHero({ topic: { primaryEntity: "Selena Gomez", title: "x", gossipType: "general" }, article: { title: "x" }, bundle },
    { getPersonImagesImpl, getTitleImagesImpl, visionImpl: async (imgs) => ({ ranked: imgs.map((_, i) => i === 0 ? { index: 0, usable: false, impact: 1, fit: 1, why: "generic model, not the subject" } : { index: i, identityMatch: true, impact: 8, fit: 8 }) }), ogImpl });
  check("wrong-person source photo rejected → does NOT publish it", h?.src !== "https://cdn.pagesix.com/generic-model.jpg");
}
{
  // TMDB path + vision ERROR → only the person's OWN profile is safe, never a title backdrop (a random movie scene)
  const h = await pickHero({ topic: { primaryEntity: "Selena Gomez", title: "x", gossipType: "general" }, article: { title: "x" }, bundle: { sources: [] } },
    { getPersonImagesImpl, getTitleImagesImpl, visionImpl: async () => { throw new Error("vision down"); }, ogImpl: async () => null });
  check("TMDB + vision error → the person's profile, never a title backdrop", h === null || h.src.includes("prof"), h?.src);
}
