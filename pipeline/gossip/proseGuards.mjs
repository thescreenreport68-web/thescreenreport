...
// 2026-07-25 — WHY FOUR ABSENCE CLAIMS SHIPPED LIVE. The escape above was matching words that belong to
// the CLAIM, not to any attribution of it: "No other outlets have REPORTED…" and "no REPRESENTATIVE has
// issued…" both looked "attributed" and were kept. A sentence that OPENS on a bare negation is the
// pipeline asserting an absence in its own voice — no speaker is credited — so the escape must not apply.
// Genuine attributed absences ("No charges were filed, according to police") keep it via EXPLICIT_ATTRIB.
const BARE_ABSENCE_OPENER = /^\s*(?:and\s+|but\s+)?(?:no|none|neither|nobody|no one|nothing|there (?:is|are|was|were) no)\b/i;
const EXPLICIT_ATTRIB = /\b(?:according to|per (?:court|police|a |an |the )|told [A-Z]|told (?:us|the|reporters)|said in a statement|(?:police|prosecutors|officials|the (?:court|filing|report|outlet))\s+(?:said|say|confirmed))\b/;
const isReportedAbsence = (t) => carriesQuote(t) || EXPLICIT_ATTRIB.test(t) || (!BARE_ABSENCE_OPENER.test(t) && ATTRIBUTED.test(t));
