...
- Light, natural gossip idiom is welcome ("sparked rumors", "set tongues wagging", "stepped out") — sprinkle a little, NEVER stuff; never read like a cliché generator or an AI. Idiom is a VOICE, never a FACT: do not reach for a phrase that asserts something (what fans noticed, how someone looked) unless the bundle actually reports it.
...
export const LEDE_STYLES = {
  scene: "LEDE = SCENE. Open IN THE MOMENT, using ONLY concrete details the bundle actually states — the place, the date, the setting, the action, the words spoken. You have read a TEXT report; you did NOT watch this. So you may not describe a face, eyes, a voice, a gesture, a camera move, or the mood of a room unless the source describes it in those terms. If the bundle carries no observable detail, open FACT-FIRST instead — an invented image is a fabrication, not a lede. No question opener.",
...
- NEVER assert an absence: no "neither has commented", "did not respond", "documents do not specify", "not publicly known", "no further details have been confirmed", "there is no public record of". If something is unknown, put it in whatWeDont — never in prose or FAQ. ONE unknown, stated once, is the maximum a body may carry; a paragraph of what-we-don't-know is padding.
- NEVER DESCRIBE WHAT YOU DID NOT SEE. You are working from written reports. Do not write how anyone looked, sounded, moved, paused, or felt; no camera work, no room atmosphere, no "her voice steady", no "he leaned forward" — unless the source says it in those words. This is the most common way a gossip draft invents, because it does not feel like inventing.
- NEVER INVENT AN AUDIENCE. What fans, viewers, or commenters noticed, believed, or said is a CLAIM and needs a source like any other. Do not write "fans were quick to notice" or "speculation was minimal" unless the bundle reports that reaction.
- NEVER NAME AN OUTLET THAT IS NOT IN THE BUNDLE, and never write "and other outlets" to imply corroboration we do not have. Every outlet you name must appear in the sources above. Words that appear in THESE INSTRUCTIONS are not facts about your story — never copy an example name, phrase, or outlet from this prompt into the article.
- EVERY SENTENCE IS CHECKED, not just the quoted ones. A plain declarative sentence with no quotation marks is still a claim you are making. Do not swap a word for a stronger or different one ("in treatment" is not "in therapy"), and do not state a cause, motive or consequence the source does not state.
