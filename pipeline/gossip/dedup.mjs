...
async function defaultAdjudicate(aSummary, bSummary) {
  const { data } = await chat({
    ...
    user: `A (already published): ${aSummary}\nB (new candidate): ${bSummary}\n\nIs B the SAME event as A (just reworded), an UPDATE (a genuine NEW development on the same story), or a DISTINCT event? If B only rewords A with no new fact, that is DUPLICATE. Return {"verdict":"DUPLICATE"|"UPDATE"|"DISTINCT","newFact":"the new development if UPDATE, else empty"}.`,
