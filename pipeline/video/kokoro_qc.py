...
def norm(t):
    ...
    toks = [w.replace("'", "") for w in re.sub(r"[^a-z0-9'\- ]", " ", t).replace("-", " ").split() if w]
    return [w[:-1] if len(w) > 3 and w.endswith("s") else w for w in toks]  # uniform s-strip both sides (possessives/plurals)
