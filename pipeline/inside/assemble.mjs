...
import { CONTENT_DIR, INSIDE_FORMAT_TAG, INSIDE_AUTHOR_SLUG, AI_DISCLOSURE, MONITOR_WINDOW_HOURS, FORMS, routeForTrigger } from "./config.inside.mjs";
...
export function buildInsideMarkdown({ article, trigger, angle, factBlock, image, dateISO }) {
  const route = routeForTrigger(trigger);
  ...
  const reactions = (article.reactionsRender || [])
    ...
    .map((r) => clean({
      speaker: r.speaker || (angle.form === "fan-pulse" ? "A fan" : ""),
  ...
  const fm = clean({
    ...
    fanConsensus: angle.form === "fan-pulse" ? article.fanConsensus || undefined : undefined,
    tweetIds: factBlock.tweetIds.length ? factBlock.tweetIds : undefined,
    updatedCount: 0,
    // Homepage placement contract. Non-flagship siblings run 5 under the parent's priority so one
    // event's angle set never monopolizes the fold.
    ...
    // Mirrors the PARENT event's verification honestly: a DEVELOPING parent gets a DEVELOPING
    // child, never a manufactured CONFIRMED badge. (confirmedOnly classes — deaths — can only
    // reach here CONFIRMED at all.)
    storyStatus: trigger.status === "CONFIRMED" ? "CONFIRMED" : "DEVELOPING",
    sensitivity: trigger.sensitivity !== "normal" ? trigger.sensitivity : undefined,
    developing: true, // reaction waves build — the monitor tops this article up
    aiDisclosure: AI_DISCLOSURE,
    dateModified: dateISO,
    provenance: clean({
      parentEventSlug: trigger.parentEventSlug || "",
      primaryEntity: trigger.primaryEntity || "",
      eventType: trigger.eventType || "other",
      sensitivity: trigger.sensitivity || "normal",
      status: trigger.status === "CONFIRMED" ? "CONFIRMED" : "DEVELOPING",
      monitor: true,
      monitorWindowH: MONITOR_WINDOW_HOURS,
      namedVoices: factBlock.stats.namedVoices,
      publishedAt: dateISO,
    }),
    ...(image ? {
      image: image.image,
